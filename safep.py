 ...
 #       plt.yticks([])
    ...
    def plot_axial_force(self):
        ...
        for i,e in enumerate(self.element):
            ...
            N=e.EA*((local_u1-local_u0)/e.length-e.alpha*e.T0)
            print('#element:',i,N)
            str_N = "{:.8g}".format(N)
    ...
    def plot_strech(self):
        print('strech')
        for i,e in enumerate(self.element):
            local_u0=e.cs*self.node[e.node[0]].u+e.sn*self.node[e.node[0]].v
            local_u1=e.cs*self.node[e.node[1]].u+e.sn*self.node[e.node[1]].v
            strech=local_u1-local_u0
            print('#element:',i,strech)
            str_strech = "{:.8g}".format(strech)
            x=(self.node[e.node[0]].x+self.node[e.node[1]].x)*0.5
            y=(self.node[e.node[0]].y+self.node[e.node[1]].y)*0.5
            self.ax.text(x,y,str_strech)
    ...
    def plot_hinge(self):
        if(self.hinge_shift==True):
            for i,e in enumerate(self.element):
                ...
                hinge_scale=0.3
                if(e.hinge[0]==True):
                    xh=self.hinge_loc*self.node[node0].x+(1.0-self.hinge_loc)*self.node[node1].x
                    yh=self.hinge_loc*self.node[node0].y+(1.0-self.hinge_loc)*self.node[node1].y
                    circle = plt.Circle((xh,yh), self.hinge_size, color='black', fill=False)
                    self.ax.add_artist(circle)
                #circle = plt.Circle((xh,yh), 0.04, color='white', fill=True)
                #ax.add_artist(circle)
                if(e.hinge[1]==True):
                    xh=(1.0-self.hinge_loc)*self.node[node0].x+self.hinge_loc*self.node[node1].x
                    yh=(1.0-self.hinge_loc)*self.node[node0].y+self.hinge_loc*self.node[node1].y
                    circle = plt.Circle((xh,yh), self.hinge_size, color='black', fill=False)
                    self.ax.add_artist(circle)
                    #circle = plt.Circle((xh,yh), 0.04, color='white', fill=True)
                    #ax.add_artist(circle)
